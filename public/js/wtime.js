define('wtime', ['jquery'], function ($) {
    ...
    /**
     * Инициализирует работу с данными времени работы
     * @param {string} name Название поля
     * @param {string} title Тайл блока со временем
     * @param {object} disableSelector Селектор ответсвенный за отключение/включение
     * @returns {object}
     */
    self.init = function (name, title, disableSelector) {
        ...
        var draw = function () {
            ...
            for (var day in matrix) {
                ...
                if (!isHeadInit) {
                    ...
                    for (var index in dayMatrix) {
                        ...
                        html += '<div class="wtb_item wtb_hour wtb_c_' + hour + '"><label>' + hourStr + '<br><input type="checkbox" value="' + hour + '" name="wtb_col_' + hour + '" class="wtb_check_hour"></label></div>';
                    }
                    html += '</div>';
                ...
                for (var index in dayMatrix) {
                    ...
                    if (!isFirstInit) {
                        html += '<div class="wtb_item wtb_day wtb_r_' + day + '"><label>' + dayOfWeek + ' <input type="checkbox" value="' + day + '" name="wtb_row_' + day + '" class="wtb_check_day"></label></div>';
                        isFirstInit = true;
                    }
            ...
            if (!isDrawed) {
                ...
                params.element = body.find('#' + name + '_wtime_block');
            } else {
